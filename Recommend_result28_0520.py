...
category_dictionary_nump=np.array(category_dictionary)
category_dict={}
category_dictRev={}
for i in category_dictionary_nump:
    category_dict.update({i[0]:i[1]})
    category_dictRev.update({i[1]:i[0]})
...
with  tf.Session() as  sess:
    ...
    while(1):
        ...
        category_name_result.append([category_dictRev[int(char)] for char in category_id_result[0]])
...
#        print('recommendation categoyr id: {}'.format(category_ID))
        ...
        Prob  = candiModel.getProb(sess, getUIJ_for_hist(memHist))
        ...
        for i in range(len(cands[0])):
            if cands[0][i] in memHist:
                continue
            else:
                if len(category_id_result[0])>0:
                    if str(orirefid_cateid_dict[cands[0][i]]) in category_id_result[0]:
                        recommendation_result.append(cands[0][i])
                        recommendation_prob.append(Prob[0][i])
                        print(i)
                        a[orirefid_cateid_dict[cands[0][i]]]=a[orirefid_cateid_dict[cands[0][i]]]+1
                        if a[orirefid_cateid_dict[cands[0][i]]]==7:
                            category_id_result[0].remove(str(orirefid_cateid_dict[cands[0][i]]))
                else:
                    break
#        print('recommendation result: {}'.format(recommendation_result))
        print('Recommending  category:')
        for n in range(len(category_name_result)):
            print(category_name_result[n])
        print('Recommending  Item:')
        for m in range(len(recommendation_result)):
            print(classname_ID_dictRev.get(recommendation_result[m]))   
            print(recommendation_prob[m])
